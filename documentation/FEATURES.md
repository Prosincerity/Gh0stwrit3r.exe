...
- [ ] **Offline beat / media player** — in-editor background audio player for instrumentals while songwriting. Built strictly using Android framework's built-in AOSP `MediaPlayer` (no heavy external libraries). Controls for play/pause, loop toggle, seek bar, and volume.
- [ ] **Instrumentals library & project beat management** — global folder (e.g. `Music/Gh0stwrit3r/Instrumentals/` or user-accessible directory) where users drop beats; assigning a beat copies/stores it directly into the project directory so projects stay self-contained.
- [ ] **Project metadata (`project.json`)** — JSON file in each project directory storing musical key, BPM, assigned beat file, title, and timestamps, using Android's built-in `org.json` (no third-party JSON libraries).
